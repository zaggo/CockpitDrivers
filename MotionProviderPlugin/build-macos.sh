#!/bin/bash
# filepath: /Users/zaggo/Developer/CockpitDrivers/MotionProviderPlugin/build.sh
...
if [ "$1" == "debug" ]; then
    BUILD_TYPE="Debug"
    echo -e "${YELLOW}╔════════════════════════════════════════╗${NC}"
    echo -e "${YELLOW}║   Motion Plugin - DEBUG Build             ║${NC}"
    echo -e "${YELLOW}╚════════════════════════════════════════╝${NC}\n"
else
    echo -e "${YELLOW}╔════════════════════════════════════════╗${NC}"
    echo -e "${YELLOW}║   Motion Provider Plugin - Build Script   ║${NC}"
    echo -e "${YELLOW}╚════════════════════════════════════════╝${NC}\n"
fi
...
if [ -d "$XPLANE_PLUGINS_DIR" ] && [ -z "$BUILD_ALL_MODE" ]; then
    ...
    echo -e "${BLUE}  cp ${OUTPUT_DIR}/mac.xpl /Volumes/1TBSSD/XPlane/X-Plane\ 12/Resources/plugins/MotionProvider/64/mac.xpl${NC}"
fi
