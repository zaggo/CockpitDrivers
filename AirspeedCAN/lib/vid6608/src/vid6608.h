 ...
 *
 * LOCAL FORK (RPMGaugeCAN):
 *   loop() is now non-blocking. The original step() busy-waited the whole
 *   accel delay (delayMicroseconds(delayUs)) on every step, so any other
 *   loop() work (OLED I2C bursts, CAN) landed *after* that wait and stretched
 *   the step period, making the needle stutter while the odometer animated.
 *   Here loop() only emits a short step pulse and returns; the inter-step
 *   interval is timed with micros(), so OLED/CAN work overlaps the wait
 *   instead of adding to it. Homing (zero()) stays blocking - it runs once at
 *   startup and wants tight sequential steps.
 */
...
class vid6608 {
  public:
     ...
     * @note LOCAL FORK: non-blocking. Emits at most one short step pulse per
     * call and only when the current accel delay has elapsed (timed via
     * micros()). Returns immediately otherwise, so callers can interleave
     * other time-critical work (OLED I2C, CAN) without stretching the step
     * period. Call it as often as possible.
     */
    void loop();
  ...
  private:
    ...
    uint32_t lastStepMicros;        // LOCAL FORK: micros() timestamp of the last emitted step pulse
    // Blocking step, used by zero() homing only
    void step(MoveDirection direction, uint16_t delayUs);
    // LOCAL FORK: short non-blocking step pulse, used by loop()
    void stepPulse(MoveDirection direction);
