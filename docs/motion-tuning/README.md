...
| `sat_surge`, `sat_sway` | % of unpaused ticks with the translational onset clamp engaged | Same reading as `sat_heave`. A high value means the onset channel is running into its per-axis limit and the cue is being flattened rather than shaped — lower the gain before raising the limit. The limit bounds this channel's own contribution; it does not guarantee `clampToReachable` never engages (measured near-worst-case headroom can fall below the limit — see `docs/motion-tuning/baseline-metrics.md`), and `sat_envelope` is what detects the case where it does. |
